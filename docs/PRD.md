...
- Filtro Servlet para extrair `x-correlationId` do header de entrada e popular o MDC.
...
- Geração de `x-correlationId` quando ausente no header (responsabilidade do chamador ou gateway).
...
1. Cliente envia request com header `x-correlationId: <UUID>`.
2. Filtro extrai o UUID e popula o MDC com a chave `correlationId`.
...
- Virtual threads habilitadas (`spring.threads.virtual.enabled=true`): verificar compatibilidade do MDC com virtual threads no Java 21.
